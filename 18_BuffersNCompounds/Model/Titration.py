...
class SpecData:
    def __init__(self, path,save=False):
        ...
        self.substrateCols = self.regex_substrates('arac')
    ...
    def regex_substrates(self,expression):
    ...
    def regex_substrateConcs(self,col_headers):
        vols =  col_headers[1:].str.extract('(\d+)').astype(float)[0] # uls
        concs = (10_000*vols)/1000 # uM and ul
        concs = pd.Series([0.0]).append(concs)
        return concs
    
    def GaussianFilterData(self):
        data = pd.DataFrame(ndimage.gaussian_filter(self.data,1))
        data.columns = self.wavelengths[:-1]
        return data
