...
class dataset:
    ...
    def plot_traces(self, save):
        ...
        if save==True:
            plt.savefig(self.path+'_PLOT.png')
        else:
            plt.show()
    def CalcRZ(self):
        #copy of the data
...
def main():
    ...
    print('RZ values:')
    print(th.md_table(pd.DataFrame(Dataset.CalcRZ()),showindex=True))
    print('\n Concentrations')
    print(th.md_table(pd.DataFrame(Dataset.calc_conc()),showindex=True))
