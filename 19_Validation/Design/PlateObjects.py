...
class Block():
    def __init__(self, Compound,DMSO):
        self.WorkingVol = 20 *1000 # convert to nl
    ...
    def MakeTransfer(self):
        ...
        for vol_cpd, vol_dmso ,testwell, blankwell in zip(vols[self.Compound], vols[self.DMSO],self.TestWells, self.BlankWells):
            ...
            for i,j in zip(cpd_transfer, dmso_transfer):
                ...
                temp = pd.DataFrame([[i,testwell,cpd_transfer[i]],\
                [j,blankwell,dmso_transfer[j]]],columns = ['SrcWell','Destination Well','Volume'])
