...
file = '20190701.CSV'
protconc=15.14 #27.56 #7.09, 14.09
count=0
#generate_markdown_Table(titles)
for i in tqdm(range(1,len(titles)+1)):
    data, pureprotein_andDMSO, diffdiff = Data_Munjing_pipeline(file,i)
    #df =df.append(return_metrics(diffdiff,concs,titles[count]))
    plotPlateData(data,str(protconc)+' uM BM3 '+titles[count],concs,save=False)
    plotPlateDifferenceSpectra(data,pureprotein_andDMSO,str(protconc)+' uM BM3 '+titles[count],concs,save=False)
    Plot_MichaelisMenten(diffdiff, concs,str(protconc)+' uM BM3 '+titles[count],save=False)
    count+=1
