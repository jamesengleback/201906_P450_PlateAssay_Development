...
def plotPlateData(data, title, concs,save=False):
    ...
    '''
    axmax = data.loc[:,390:].max().max()*1.1 # breathing room
...
protconc=14.09 #27.56 #7.09, 14.09
count=0
#generate_markdown_Table(titles)
i=titles[1]
data, pureprotein_andDMSO, diffdiff = Data_Munjing_pipeline(file,i)
df =df.append(return_metrics(diffdiff,concs,i))
plotPlateData(data,str(protconc)+' uM BM3 '+i,concs,save=False)
